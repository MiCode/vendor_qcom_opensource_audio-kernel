		...
		if (card) {
			...
			if (pdata) {
				if (wcd_mbhc_cfg.use_fsa4476_gpio != 0) {
					...
					if (pdata->fsa_handle)
						ucontrol->value.integer.value[0] = fsa4480_get_direction(pdata->fsa_handle);
				}
			}
		}
	}
...
static struct snd_soc_dai_link sec_mi2s_be_dai_links[] = {
	{
...
static struct snd_soc_dai_link msm_tavil_dai_links[
			 ...
			 ARRAY_SIZE(ext_disp_be_dai_link) +
			 ARRAY_SIZE(msm_mi2s_be_dai_links) +
			 ARRAY_SIZE(sec_mi2s_be_dai_links) +
...
static struct snd_soc_card *populate_snd_card_dailinks(struct device *dev)
{
	...
	if (!strcmp(match->data, "pahu_codec")) {
		...
		if (!ret && val) {
			...
			printk("<%s> <%d>: get_hw_version_platform = %d.\n", __func__, __LINE__, get_hw_version_platform());

			if (get_hw_version_platform() == HARDWARE_PLATFORM_CRUX) {
				memcpy(msm_tavil_dai_links + total_links,
					sec_mi2s_be_dai_links,
					sizeof(sec_mi2s_be_dai_links));
				total_links += ARRAY_SIZE(sec_mi2s_be_dai_links);
