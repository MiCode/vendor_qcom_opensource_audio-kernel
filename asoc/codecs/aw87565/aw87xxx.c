...
#define AW87XXX_DRIVER_VERSION	"v4.0.7"
...
int aw87xxx_update_profile(struct aw87xxx *aw87xxx, char *profile)
{
	...
	AW_DEV_LOGI(aw87xxx->dev, "load profile[%s] enter", profile);
...
int aw87xxx_set_profile(int dev_index, char *profile)
{
	...
	list_for_each(pos, &g_aw87xxx_list) {
		aw87xxx = list_entry(pos, struct aw87xxx, list);
		if (aw87xxx->dev_index == dev_index) {
			AW_DEV_LOGI(aw87xxx->dev, "set dev_index = %d, profile = %s",
				dev_index, profile);
...
extern int get_frsm_probe_complete_flag(void);
...
#endif
{
...
#if IS_ENABLED(CONFIG_SND_SOC_FS181X)
	int frsm_probe_complete = get_frsm_probe_complete_flag();

	aw87xxx_i2c_probe_cnt++;
	if(frsm_probe_complete != 0 || (aw87xxx_i2c_probe_cnt > 500)) {
		//continue
	} else {
		//waiting
		return -EPROBE_DEFER;
	...
	if(frsm_probe_complete == 1) {
		AW_DEV_LOGE(&client->dev, "fs1815cn probe success, aw87xxx always fail");
		return -ENODEV;
	}
#endif /* CONFIG_SND_SOC_FS181X */
...
#if IS_ENABLED(CONFIG_SND_SOC_FS181X)
MODULE_SOFTDEP("pre: fs181x_drv_dlkm");
#endif /* CONFIG_SND_SOC_FS181X */
