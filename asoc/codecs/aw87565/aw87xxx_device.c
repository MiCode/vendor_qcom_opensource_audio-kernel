...
static int aw_send_cache(struct aw_device *aw_dev, struct aw_reg_cache *cache)
{
    ...
    AW_DEV_LOGD(aw_dev->dev, "reg=0x%02x, count = %d", cache->start_addr, cache->count);
	...
	struct aw_data_container *profile_data)
{
	...
	for (i = 0; i < profile_data->len; i = i + 2) {
		reg_addr = profile_data->data[i];
		reg_val = profile_data->data[i + 1];
		AW_DEV_LOGD(aw_dev->dev, "reg=0x%02x, val = 0x%02x", reg_addr, reg_val);
...
int aw87xxx_dev_hw_pwr_ctrl_cnt = 0;
...
void aw87xxx_dev_hw_pwr_ctrl(struct aw_device *aw_dev, bool enable)
{
	...
	if(aw87xxx_dev_hw_pwr_ctrl_cnt > 2)
		aw87xxx_dev_hw_pwr_ctrl_cnt  = 2;
	if(aw87xxx_dev_hw_pwr_ctrl_cnt < 0)
		aw87xxx_dev_hw_pwr_ctrl_cnt  = 0;
	if (enable) {
		if (aw87xxx_dev_hw_pwr_ctrl_cnt == 0) {
		...
		aw87xxx_dev_hw_pwr_ctrl_cnt ++;
		AW_DEV_LOGI(aw_dev->dev, "aw87xxx_dev_hw_pwr_ctrl_cnt = %d", aw87xxx_dev_hw_pwr_ctrl_cnt);
