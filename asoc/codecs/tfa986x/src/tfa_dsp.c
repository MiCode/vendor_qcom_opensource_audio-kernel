...
enum Tfa98xx_Error tfa_dsp_msg_read(struct tfa_device *tfa, int length24, unsigned char *bytes24)
{
	...
	if (tfa->has_msg == 0) /* via i2c */ {
		if (tfa->dev_ops.tfa_dsp_msg_read) {
...
error_exit:
	...
	/* When opening the device, exit the high-impedance state, unless the target profile is standby mode */
...
enum tfa_error tfa_dev_stop(struct tfa_device *tfa)
{
	...
	/* When the device stops, set register 0xc050 to 1 to put the PA output into a high-impedance state */
