...
static enum tfa_error tfa98xx_tfa_start(struct tfa98xx *tfa98xx, int next_profile, int vstep)
{
	...
	/* Get currentprofile */
	active_profile = tfa_dev_get_swprof(tfa98xx->tfa);
	if (active_profile == 0xff) {
		active_profile = -1;
	}

	pr_info("%s active_profile=%d next_profile=%d  vstep=%d\n", __func__, active_profile, next_profile, vstep);
	if (trace_level & 8) {
		start_time = ktime_get_boottime();
	}
	err = tfa_dev_start(tfa98xx->tfa, next_profile, vstep);
	//pr_info("%s  after performed tfa_dev_start return (%d)\n", __func__, err);
#if IS_ENABLED(CONFIG_MIEV)
    if(err && (active_profile != -1) && (next_profile != 0)) {
        pr_err("%s tfa_dev_start fail[TF-NOSOUND], err=%d\n", __func__, err);
        mievent_report(906001354, "PA data exception", tfa98xx->dev);
	}
#endif
	if (trace_level & 8) {
...
static int tfa98xx_spksw_gpio_get(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *codec = snd_soc_kcontrol_component(kcontrol);
	struct tfa98xx *tfa98xx = snd_soc_component_get_drvdata(codec);

	int current_level = gpio_get_value_cansleep(tfa98xx->spk_sw_gpio);
	dev_info(&tfa98xx->i2c->dev, "spk_sw level cached = %d, actual = %d\n",
					tfa98xx->spk_sw_level, current_level);

	if (current_level < 0) {
		// failed to read the actual value, fallback to cached value
		ucontrol->value.enumerated.item[0] = tfa98xx->spk_sw_level;
	} else {
...
static int tfa98xx_spksw_gpio_put(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *codec = snd_soc_kcontrol_component(kcontrol);
	struct tfa98xx *tfa98xx = snd_soc_component_get_drvdata(codec);
	int spk_sw_gpio = ucontrol->value.enumerated.item[0];

	dev_info(&tfa98xx->i2c->dev, "spk_sw = %d\n", spk_sw_gpio);

	gpio_set_value_cansleep(tfa98xx->spk_sw_gpio, !!spk_sw_gpio);

	return 0;
}

static const struct snd_kcontrol_new tfa98xx_spksw_controls[] = {
	SOC_SINGLE_EXT("tfa98xx_spksw_gpio_switch", SND_SOC_NOPM, 0, 0x3, 0,
		tfa98xx_spksw_gpio_get, tfa98xx_spksw_gpio_put)
};
...
static int tfa98xx_create_controls(struct tfa98xx *tfa98xx)
{
	...
	ret = snd_soc_add_component_controls(tfa98xx->codec, tfa98xx_spksw_controls, ARRAY_SIZE(tfa98xx_spksw_controls));
	pr_info("create tfa98xx_spksw_controls  ret=%d", ret);
...
static void tfa98xx_container_loaded(const struct firmware *cont, void *context)
{
	...
	*/
	...
	if (tfa_dev_probe(tfa98xx->i2c->addr, tfa98xx->tfa) != 0) {
		dev_err(tfa98xx->dev, "Failed to probe TFA98xx @ 0x%.2x [TF-NOSOUND]\n", tfa98xx->i2c->addr);
#if IS_ENABLED(CONFIG_MIEV)
		mievent_report(906001353,"PA i2c exception",&tfa98xx->i2c->dev);
#endif
		return;
	}
...
static int tfa98xx_load_container(struct tfa98xx *tfa98xx)
{
	...
	return request_firmware_nowait(THIS_MODULE, true,
		...
		tfa98xx, tfa98xx_container_loaded);//modify for kernel6.x
}
...
static int tfa98xx_parse_dt(struct device *dev, struct tfa98xx *tfa98xx,
	...
	if (tfa98xx->reset_gpio < 0)
		dev_info(dev, "No reset GPIO provided, will not HW reset device\n");

	tfa98xx->irq_gpio = of_get_named_gpio(np, "irq-gpio", 0);
	if (tfa98xx->irq_gpio < 0)
		dev_info(dev, "No IRQ GPIO provided.\n");
	else
		dev_info(dev, "IRQ found: %d\n", tfa98xx->irq_gpio);
	...
	if (tfa98xx->spk_sw_gpio < 0)
		dev_info(dev, "No spk_sw_gpio GPIO provided\n");
	return 0;
}
...
static int tfa98xx_i2c_probe(struct i2c_client *i2c) //modify for kernel6.X
{
	...
	if (gpio_is_valid(tfa98xx->reset_gpio)) {
		...
		if (ret) {
			dev_err(&i2c->dev, "Failed to request reset GPIO\n");
			if (ret == -EBUSY)
				return -EPROBE_DEFER;
		}
	}
	...
	if (gpio_is_valid(tfa98xx->spk_sw_gpio)) {
		...
		if (ret) {
			dev_err(&i2c->dev, "Failed to request spk_sw_gpio\n");
			if (ret == -EBUSY)
				return -EPROBE_DEFER;
		}
	}
	...
	if ((no_start == 0) && (no_reset == 0)) {
		while (retry_count > 0) {
			...
			if (ret >= 0){
				pr_info("%s: retry_regmap_read success\n", __func__);
				break;
			}
			mdelay(3);
		}
	...
	set_smartpa_type("tfa98xx", sizeof("tfa98xx"));
	...
	if (ret) {
		dev_info(&i2c->dev, "error creating sysfs files [TF-NOSOUND]\n");
#if IS_ENABLED(CONFIG_MIEV)
	mievent_report(906001353,"PA i2c exception",&i2c->dev);
#endif
	}
	set_smartpa_codec(&i2c->dev, dai->name);
...
void tfa98xx_i2c_remove(struct i2c_client *i2c) //modify for kernel6.X
{
	struct tfa98xx *tfa98xx = i2c_get_clientdata(i2c);
...
int mievent_report(unsigned int eventid, const char *value, struct device *dev)
{
	...
	i2c_len = strlen(dev->kobj.name) + 1;
	i2c_info = kmalloc(i2c_len, GFP_KERNEL);
	if (!i2c_info) {
		dev_err(dev, "%s: Failed to allocate memory for i2c_info\n", __func__);
		return -ENOMEM;
	}
	snprintf(i2c_info, i2c_len, "%s", dev->kobj.name);
	dev_info(dev, "%s: reg = %s, KeyWord = %s DFS report\n", __func__, i2c_info,value);
	mievent = cdev_tevent_alloc(eventid);
	if (!mievent) {
		dev_err(dev, "%s: Failed to allocate mievent\n", __func__);
        	kfree(i2c_info);
        	i2c_info = NULL;
		return -ENOMEM;
	}
...
#if defined (CONFIG_SND_SOC_SIA91XX)
MODULE_SOFTDEP("pre: sia91xx_dlkm");
#endif
