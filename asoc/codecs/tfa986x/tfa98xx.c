	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	if (strstr(tfa_cont_profile_name(tfa98xx, prof_idx), "ultrasound") != NULL){
		tfa98xx_mixer_profile = profile;
		pr_info("ultrasound profile, do not update tfa98xx_mixer_profile\n");
	} else {
	...
	list_for_each_entry(tfa98xx, &tfa98xx_device_list, list) {
		...
		if (strstr(tfa_cont_profile_name(tfa98xx, prof_idx), "ultrasound") != NULL){
			pr_info("ultrasound profile, do not update tfa98xx->profile\n");
		} else {
		...
		if (ready) {
			...
			if( (strcmp(tfaContDeviceName(tfa98xx->tfa->cnt, tfa98xx->tfa->dev_idx), "SPK") == 0) && (strstr(tfa_cont_profile_name(tfa98xx, prof_idx), "ultrasound") != NULL)){
				pr_info("tfa : SPK && ultrasound profile,skip start the device\n");
			}else{
				if ((strstr(tfa_cont_profile_name(tfa98xx, prof_idx), "ultrasound") != NULL)){
					if (force) {
						...
						//tfa98xx_mixer_profile = profile;
						pr_info("RCV is system ready and no force, reopen no need, nothing to do\n");
					}
                                }  else {
					err = tfa98xx_tfa_start(tfa98xx, prof_idx, tfa98xx->vstep);
				}
...
static int tfa98xx_i2c_probe(struct i2c_client *i2c,
    const struct i2c_device_id *id) //modify for kernel6.X
{
