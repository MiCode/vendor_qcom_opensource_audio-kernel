...
#define PINCTRL_SELECT_RETIES	5
/* error list */
...
#define SIA81XX_DRV_NAME					"sia8001"
...
int wcd937x_no_sia81xx(void)
{
	return g_sipa_chip_status;
}
EXPORT_SYMBOL(wcd937x_no_sia81xx);
/* lct_audio add end */
...
static int check_sipa_status(sipa_dev_t *si_pa)
{
	...
	if (1 == si_pa->en_dyn_id) {
		...
		gpio_direction_output(si_pa->id_pin, 0); // set gpio low
...
static int sipa_probe(struct platform_device *pdev)
{
	...
	/* ADD AUDIO PA INFO BEGIN */
#ifdef CONFIG_LCT_AUDIO_INFO
	lct_audio_info_set_pa_name(SIA81XX_DRV_NAME, strlen(SIA81XX_DRV_NAME));
#endif
	/* ADD AUDIO PA INFO END */
	return 0;
