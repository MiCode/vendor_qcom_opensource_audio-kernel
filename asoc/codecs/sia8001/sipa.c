...
/* lct_audio add begin */
static sipa_dev_t *g_wcd93xx_sia_dev;
int g_sipa_chip_status = -EINVAL;
/* lct_audio add end */
...
/* lct_audio add begin */
int wcd93xx_siaxx_resume(void)
{
	return sipa_resume(g_wcd93xx_sia_dev);
}
EXPORT_SYMBOL(wcd93xx_siaxx_resume);
int wcd93xx_siaxx_suspend(void)
{
	return sipa_suspend(g_wcd93xx_sia_dev);
}
EXPORT_SYMBOL(wcd93xx_siaxx_suspend);
int wcd93xx_no_siaxx(void)
{
	return g_sipa_chip_status;
}
EXPORT_SYMBOL(wcd93xx_no_siaxx);
/* lct_audio add end */
...
static int check_sipa_status(sipa_dev_t *si_pa)
{
	...
	gpio_direction_output(si_pa->id_pin, 0);

	return ret;
}
#endif

static int distinguish_chip_type(sipa_dev_t *si_pa)
{
#ifdef DISTINGUISH_CHIP_TYPE
...
static int sipa_probe(struct platform_device *pdev)
{
	...
	if (distinguish_chip_type(si_pa))
	{
		ret = -ENODEV;
		goto out0;
	}
	...
	#ifdef CONFIG_LCT_AUDIO_INFO
	lct_audio_info_set_pa_name(SIPA_DRV_NAME, strlen(SIPA_DRV_NAME));
	#endif
	/* lct_audio add begin */
	g_sipa_chip_status = 1;
	g_wcd93xx_sia_dev = si_pa;
	/* lct_audio add end */

	return 0;
	
out0:
    g_sipa_chip_status = 0;
	put_sipa_dev(si_pa);
...
//module_init(sipa_pa_init); /* module_init after fsm */
