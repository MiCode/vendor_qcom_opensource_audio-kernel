...
static int aw882xx_gpio_request(struct aw882xx *aw882xx)
{
	...
	if (gpio_is_valid(aw882xx->irq_gpio)) {
		...
		if (ret) {
			aw_dev_err(aw882xx->dev, "int request failed, ret %d", ret);
			return ret;
		}
	}
	...
	if (gpio_is_valid(aw882xx->spksw_gpio)) {
		...
		if (ret) {
			aw_dev_err(aw882xx->dev, "spksw request failed, ret %d", ret);
			return ret;
		}
	}
...
#endif
{
	...
	if (ret) {
		aw_dev_err(&i2c->dev, "aw882xx_gpio_request failed ret=%d, -EBUSY %d", ret, -EBUSY);
		if (ret == -EBUSY)
			return -EPROBE_DEFER;
	}
	...
	if (ret < 0) {
		...
		memset(&aw_dummy_codec, 0, sizeof(struct snd_soc_component_driver));
		dummy_dai_drv = &aw_dummy_dai;
		dummy_dai_drv->name = devm_kstrdup(aw882xx->dev, "aw882xx-aif", GFP_KERNEL);
		if (!(dummy_dai_drv->name)) {
			aw_dev_err(&i2c->dev, "failed to duplicate string\n");
			return -ENOMEM;
		}
		ret = aw882xx_append_i2c_suffix("%s-%d-%x", &dummy_dai_drv->name, aw882xx);
		if (ret < 0) {
			aw_dev_err(&i2c->dev, "aw882xx_append_i2c_suffix failed");
		}
		ret =  snd_soc_register_component(aw882xx->dev, &aw_dummy_codec,
			&aw_dummy_dai, 1);
		if (ret < 0) {
			aw_dev_err(&i2c->dev, "%s: Register dummy codec failed\n", __func__);
		}
		return 0;
#endif
	}
