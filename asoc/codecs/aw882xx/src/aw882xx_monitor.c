...
static int aw_monitor_get_temp_and_vol(struct aw_device *aw_dev)
{
	...
	if (ret < 0)
		return ret;
#endif
	// aw_dev_info(aw_dev->dev, "chip voltage = %d, chip temperature = %d", voltage, current_temp);

	monitor->current_voltage = voltage;
	monitor->current_temp = current_temp;
...
static int aw_monitor_work(struct aw_device *aw_dev)
{
	...
	aw_dev_dbg(aw_dev->dev, "set_ipeak = 0x%x, set_gain = 0x%x, set_vmax = 0x%x, voltage: %d, temperature: %d",
			set_table.ipeak, set_table.gain, set_table.vmax, monitor->current_voltage, monitor->current_temp);
