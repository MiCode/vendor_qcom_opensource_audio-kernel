	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (ret == CALI_STR_CALI_RE_F0) {
		aw_cali_svc_cali_cmd(aw_dev, AW_CALI_CMD_RE_F0, is_single_cali, CALI_OPS_HMUTE|CALI_OPS_NOISE);
		return count;
...
static ssize_t aw_cali_class_te_show(struct  class *class, struct class_attribute *attr, char *buf)
{
	struct list_head *dev_list;
	struct aw_device *local_dev;
	int ret, i;
	ssize_t len = 0;
	int32_t cali_te[AW_DEV_CH_MAX] = {0};
	...
	ret = aw_cali_svc_get_devs_te(local_dev, cali_te, AW_DEV_CH_MAX);
	if (ret <= 0) {
		aw_dev_err(local_dev->dev, "get te failed ");
	} else {
		for (i = 0; i < ret; i++) {
			len += snprintf(buf+len, PAGE_SIZE-len, "%s:%d degrees_Celsius ", ch_name[i], cali_te[i]);
		}
		len += snprintf(buf+len, PAGE_SIZE-len, " \n");
	}
...
static struct class_attribute class_attr_te_show = \
		__ATTR(te_show, S_IRUGO, \
		aw_cali_class_te_show, NULL);
...
static void aw_cali_class_attr_init(struct aw_device *aw_dev)
{
	...
	ret = class_create_file(&aw_cali_class, &class_attr_te_show);
	if (ret)
	{
		aw_dev_info(aw_dev->dev, "creat class_attr_te_show fail");
	}
}
...
static void aw_cali_class_attr_deinit(struct aw_device *aw_dev)
{
	...
	class_remove_file(&aw_cali_class, &class_attr_te_show);
...
static int aw_cali_misc_switch_dev(struct file *filp, struct aw_device *aw_dev, char *cmd_buf)
{
	...
	for (i = 0; i < AW_DEV_CH_MAX; i++) {
		if (strstr(dev_select, ch_name[i])) {
			break;
		}
	}
