...
#define AW882XX_DRIVER_VERSION "v0.3.0.3"
...
static int aw882xx_codec_probe(aw_snd_soc_codec_t *aw_codec)
{
	...
	/*load monitor bin*/
	aw_monitor_load_firmware(&aw882xx->aw_pa->monitor_desc);
...
static int aw882xx_read_chipid(struct aw882xx *aw882xx)
{
	...
	while (cnt < AW_READ_CHIPID_RETRIES) {
		...
		default:
			aw_dev_info(aw882xx->dev, "unsupported device revision (0x%x)", reg_value);
			break;
		}
		cnt++;
...
static int aw882xx_awrw_data_check(struct aw882xx *aw882xx, int *data)
{
	...
	if ((data[AWRW_HDR_ADDR_BYTES] != AWRW_ADDR_BYTES) || (data[AWRW_HDR_DATA_BYTES] != AWRW_DATA_BYTES)) {
...
static int aw882xx_awrw_parse_buf(struct aw882xx *aw882xx, const char *buf, size_t count)
{
	...
	if (sscanf(buf, "0x%02x 0x%02x 0x%02x 0x%02x 0x%02x",
		...
		if (data[AWRW_HDR_WR_FLAG] == AWRW_FLAG_WRITE) {
			...
			aw_dev_err(aw882xx->dev, "please check str format, unsupport flag %d", data[AWRW_HDR_WR_FLAG]);
			return -EINVAL;
		}
	} else {
