...
#if IS_ENABLED(CONFIG_MIEV)
			if (mbhc->hph_type == WCD_MBHC_HPH_STEREO) {
				uint32_t z_diff = (mbhc->zl > mbhc->zr) ? (mbhc->zl - mbhc->zr) : (mbhc->zr - mbhc->zl);
				if (z_diff > HEADSET_Z_DIFF_MAX) {
					pr_warn("%s: The impedance difference is too large, zl %d, zr %d\n",
						__func__, mbhc->zl, mbhc->zr);
					mievent  = cdev_tevent_alloc(906005004);
					cdev_tevent_add_int(mievent, "MbhcStage", MBHC_DETECT_PLUG);
					cdev_tevent_add_int(mievent, "LeftImpedance", mbhc->zl);
					cdev_tevent_add_int(mievent, "RightImpedance", mbhc->zr);
...
#if IS_ENABLED(CONFIG_MIEV)
		mievent  = cdev_tevent_alloc(906005005);
		cdev_tevent_add_int(mievent, "MbhcStage", MBHC_DETECT_PLUG);
		cdev_tevent_add_int(mievent, "PlugType", plug_type);
		cdev_tevent_write(mievent);
		cdev_tevent_destroy(mievent);
#endif
	}
		      ...
		      bool impedance_det_en)
{
	...
	if (ret) {
#if IS_ENABLED(CONFIG_MIEV)
		mievent  = cdev_tevent_alloc(906005001);
		cdev_tevent_add_int(mievent, "MbhcStage", MBHC_INIT_IRQ);
		cdev_tevent_add_int(mievent, "MissingNodeType", 0);
		cdev_tevent_write(mievent);
		cdev_tevent_destroy(mievent);
#endif
		dev_err(card->dev,
	...
	if (ret) {
		...
		mievent  = cdev_tevent_alloc(906005001);
		cdev_tevent_add_int(mievent, "MbhcStage", MBHC_INIT_IRQ);
		cdev_tevent_add_int(mievent, "MissingNodeType", 1);
		cdev_tevent_write(mievent);
		cdev_tevent_destroy(mievent);
#endif
		dev_err(card->dev,
