...
static int fs15xx_pinctrl_init(struct platform_device *pdev, fs15xx_dev_t *fs15xx)
{
	...
	fs15xx->pinctrl = devm_pinctrl_get(&pdev->dev);
	if (IS_ERR(fs15xx->pinctrl)) {
	...
	fs15xx->fs15xx_id_default = pinctrl_lookup_state(fs15xx->pinctrl, "fs15xx_id_default");
	if (IS_ERR(fs15xx->fs15xx_id_default)) {
		ret |= PTR_ERR(fs15xx->fs15xx_id_default);
		fs15xx->fs15xx_id_default = NULL;
		pr_err("lookup state: fs15xx_id_default failed, ret: %d\n", ret);
	} else {
		pinctrl_select_state(fs15xx->pinctrl, fs15xx->fs15xx_id_default);
	}

	fs15xx->fs15xx_id_active = pinctrl_lookup_state(fs15xx->pinctrl, "fs15xx_id_active");
	if (IS_ERR(fs15xx->fs15xx_id_active)) {
		ret |= PTR_ERR(fs15xx->fs15xx_id_active);
		fs15xx->fs15xx_id_active = NULL;
		pr_err("lookup state: fs15xx_id_active failed, ret: %d\n", ret);
	} else {
		pr_info("lookup state: fs15xx_id_active success\n");
	}
#endif

	fs15xx->fs15xx_cmd_default = pinctrl_lookup_state(fs15xx->pinctrl, "fs15xx_cmd_default");
	if (IS_ERR(fs15xx->fs15xx_cmd_default)) {
		ret |= PTR_ERR(fs15xx->fs15xx_cmd_default);
		fs15xx->fs15xx_cmd_default = NULL;
		pr_err("lookup state: fs15xx_cmd_default failed, ret: %d\n", ret);
	} else {
		pinctrl_select_state(fs15xx->pinctrl, fs15xx->fs15xx_cmd_default);
	}
	...
	fs15xx->fs15xx_mod_default = pinctrl_lookup_state(fs15xx->pinctrl, "fs15xx_mod_default");
	if (IS_ERR(fs15xx->fs15xx_mod_default)) {
		fs15xx->fs15xx_mod_default = NULL;
		ret |= PTR_ERR(fs15xx->fs15xx_mod_default);
		pr_err("lookup state: fs15xx_mod_default failed, ret: %d\n", ret);
	} else {
		pinctrl_select_state(fs15xx->pinctrl, fs15xx->fs15xx_mod_default);
	}
#else
	fs15xx->fs15xx_mod_default = NULL;
#endif
	pr_info("fs15xx_pinctrl_init done");
...
static int fs15xx_check_pa_type(fs15xx_dev_t *fs15xx)
{
	...
	if (fs15xx->fs15xx_id_active && fs15xx->pinctrl)
	...
	if (fs15xx->fs15xx_id_default && fs15xx->pinctrl)
		pinctrl_select_state(fs15xx->pinctrl, fs15xx->fs15xx_id_default);
#else
...
#if defined(FS15XX_PINCTRL_SUPPORT)
	devm_pinctrl_put(fs15xx->pinctrl);
	fs15xx->pinctrl = NULL;
	fs15xx->fs15xx_id_default = NULL;
	fs15xx->fs15xx_id_active = NULL;
	fs15xx->fs15xx_cmd_default = NULL;
