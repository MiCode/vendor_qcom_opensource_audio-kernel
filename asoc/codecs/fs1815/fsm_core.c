...
int fsm_stub_switch_preset(fsm_dev_t *fsm_dev)
{
	...
	if ((fsm_dev->own_scene & cfg->next_scene) == 0) {
		pr_addr(debug, "fsm_dev->own_scene=%d, skip,cfg->next_scene= %d",fsm_dev->own_scene ,cfg->next_scene);
		return ret;
	}
...
static const char *const ext_pa_switch_text[] = {"disable", "enable"};
static int ext_pa_switch_enabled = 0;
static SOC_ENUM_SINGLE_EXT_DECL(ext_pa_switch, ext_pa_switch_text);

static int ext_pa_switch_get(struct snd_kcontrol *kcontrol,
                                struct snd_ctl_elem_value *ucontrol)
{
        ucontrol->value.integer.value[0] = ext_pa_switch_enabled;
        pr_debug("%s: ext_pa_switch = %ld\n", __func__,
                ucontrol->value.integer.value[0]);
        return 0;
}

static int ext_pa_switch_put(struct snd_kcontrol *kcontrol,
                                struct snd_ctl_elem_value *ucontrol)
{
        ext_pa_switch_enabled = ucontrol->value.integer.value[0];
        if (ext_pa_switch_enabled) {
                fsm_speaker_onn();
        } else {
                fsm_speaker_off();
        }
...
const struct snd_kcontrol_new fsm_snd_controls[] =
{
	...
	SOC_ENUM_EXT("EXT PA", ext_pa_switch,
			ext_pa_switch_get, ext_pa_switch_put),
};
