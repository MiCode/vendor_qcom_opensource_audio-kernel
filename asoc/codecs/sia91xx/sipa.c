...
static int sipa_91xx_reg_dump(struct sipa_dev_s *si_pa, char *buff)
{
	...
	for (i = 0; i < arrSize; i++) {
		...
		if (ret == 0) {
			off += snprintf(buff + off, 33, "reg addr = %02x, value = 0x%x \n", regAddr[i], reg);
			reg = 0;
		} else {
	...
	struct sipa_dev_s *si_pa)
{
	...
	if (is_chip_type_supported(si_pa->chip_type) &&
		...
		if (0 == si_pa->disable_pin) {
			...
			if (IS_DIGITAL_PA_TYPE(si_pa->chip_type) && !IS_DIGITAL_PA_PULL_RST_TYPE(si_pa->chip_type)) {
	...
	struct sipa_dev_s *si_pa)
{
	...
	if (is_chip_type_supported(si_pa->chip_type) &&
		...
		if(!sipa_regmap_set_chip_off(si_pa) && IS_DIGITAL_PA_TYPE(si_pa->chip_type)) {
			if (IS_DIGITAL_PA_PULL_RST_TYPE(si_pa->chip_type)) {
					gpio_set_value(si_pa->rst_pin, SIA91XX_LOW_LEVEL);
				} else {
					gpio_set_value(si_pa->rst_pin, SIA91XX_HIGH_LEVEL);
...
#endif
{
	...
	if (gpio_is_valid(si_pa->rst_pin)) {
		ret = devm_gpio_request_one(&client->dev, si_pa->rst_pin, GPIOF_OUT_INIT_LOW, "SIA91XX_RST");
		if (ret) {
	...
	if (IS_DIGITAL_PA_TYPE(si_pa->chip_type)) {
...
#if IS_ENABLED(CONFIG_MIEV)
			mievent_report(906001352, "PA detection exception", &client->dev);
#endif
			return ret;
		}
		...
		if (gpio_is_valid(si_pa->spksw_gpio)) {
			ret = devm_gpio_request_one(&client->dev, si_pa->spksw_gpio, GPIOF_OUT_INIT_LOW, "sia91xx_spksw");
			if (ret) {
...
#if IS_ENABLED(CONFIG_MIEV)
			mievent_report(906001351, "PA internal exception", &client->dev);
#endif
			return ret;
		}
...
static int sipa_property_init(struct device_node *sipa_of_node, sipa_dev_t *si_pa)
{
	...
	if (0 == si_pa->disable_pin) {
		...
		if (IS_DIGITAL_PA_TYPE(si_pa->chip_type)) {
			if(si_pa->chip_type == CHIP_TYPE_SIA5118) {
				boost_pin = of_get_named_gpio(sipa_of_node, "si,si_pa_boost", 0);
				if (boost_pin < 0) {
		...
		if (IS_DIGITAL_PA_TYPE(si_pa->chip_type)) {
			spksw_gpio = of_get_named_gpio(sipa_of_node, "spksw-gpio", 0);
			if (spksw_gpio < 0) {
				pr_err("[  err][%s] %s: spksw_gpio < 0 !!! \n", LOG_FLAG, __func__);
			}
			si_pa->spksw_gpio = spksw_gpio;
			pr_info("[  info][%s] %s: spksw_gpio = %d \n", LOG_FLAG, __func__, si_pa->spksw_gpio);
		}
...
static int sipa_pinctrl_select(struct platform_device *pdev, sipa_dev_t *si_pa)
{
	...
	if (0 == si_pa->disable_pin) {
		...
		if (IS_DIGITAL_PA_TYPE(si_pa->chip_type)) {
			if(si_pa->chip_type == CHIP_TYPE_SIA5118 && si_pa->boost_pin > 0) {
				pinctrl_state = pinctrl_lookup_state(si_pa_pinctrl, "si_pa_boost");
				if (NULL == pinctrl_state) {
				...
				if (gpio_is_valid(si_pa->boost_pin)) {
					gpio_direction_output(si_pa->boost_pin, SIA81XX_DISABLE_LEVEL);
				}
			}
			if (si_pa->irq_pin > 0) {
		...
		if (IS_DIGITAL_PA_TYPE(si_pa->chip_type)) {
			if (gpio_is_valid(si_pa->irq_pin)) {
				ret = devm_gpio_request_one(&pdev->dev, si_pa->irq_pin, GPIOF_DIR_IN, "SIA91XX_INT");
				if (ret) {
					pr_err("%s: fail to request irq gpio %d", __func__, ret);
					return ret;
				}
			}
		}
...
MODULE_SOFTDEP("pre: aw882xx_dlkm");
#endif
