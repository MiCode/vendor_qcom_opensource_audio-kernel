...
{
	fsm_config_t *cfg = fs19xx_get_config();
	int scene_index;
			...
			struct snd_ctl_elem_value *ucontrol)
{
	...
	pr_info("amp_on: %d", ucontrol->value.integer.value[0]);
			...
			struct snd_ctl_elem_value *ucontrol)
{
	...
	fsm_mutex_lock();
	fs19xx_afe_handle_direction(false, &angle);
	fsm_mutex_unlock();
...
#ifdef CONFIG_FSM_CROSSFADE
	fsm_mutex_lock();
	fs19xx_afe_handle_direction(true, &angle);
	fsm_mutex_unlock();
#else
