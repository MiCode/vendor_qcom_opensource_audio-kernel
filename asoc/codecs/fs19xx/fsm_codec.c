			...
			struct snd_ctl_elem_value *ucontrol)
{
	...
	ucontrol->value.integer.value[0] = volume;
	pr_info("fsm_ramp_volume_get volume: %d", volume);
			...
			struct snd_ctl_elem_value *ucontrol)
{
	...
	pr_info("fsm_ramp_volume_put volume: %d", volume);
...
int fsm_amp_get(struct snd_kcontrol *kcontrol,
			struct snd_ctl_elem_value *ucontrol)
{
	fsm_config_t *cfg = fsm_get_config();

	ucontrol->value.integer.value[0] = ((cfg != NULL) ? cfg->speaker_on : 0);
	pr_info("amp_on: %ld", ucontrol->value.integer.value[0]);

	return 0;
}

int fsm_amp_put(struct snd_kcontrol *kcontrol,
			struct snd_ctl_elem_value *ucontrol)
{
	fsm_config_t *cfg = fsm_get_config();
...
int fsm_ultrasound_get(struct snd_kcontrol *kcontrol,
			struct snd_ctl_elem_value *ucontrol)
{
	fsm_config_t *cfg = fsm_get_config();

	ucontrol->value.integer.value[0] = ((cfg != NULL) ? cfg->ultrasound_on : 0);

	return 0;
}

int fsm_ultrasound_put(struct snd_kcontrol *kcontrol,
			struct snd_ctl_elem_value *ucontrol)
{
	fsm_config_t *cfg = fsm_get_config();

	cfg->ultrasound_on = ucontrol->value.integer.value[0];

	pr_info("ultrasound_on: %d", cfg->ultrasound_on);

	if (cfg->ultrasound_on && (cfg->next_scene == FSM_SCENE_MUSIC)) {
		fsm_set_scene(4);
	} else if (!cfg->ultrasound_on && (cfg->next_scene == FSM_SCENE_VOIP_ULTRA)) {
		fsm_set_scene(0);
	}
...
static int fsm_spksw_gpio_get(struct snd_kcontrol *kcontrol,
			struct snd_ctl_elem_value *ucontrol)
{
	fsm_config_t *cfg = fsm_get_config();

	int current_level = gpio_get_value_cansleep(cfg->spksw_gpio);
	pr_info("spksw level cached = %d, actual = %d\n", cfg->spksw_level, current_level);

	if (current_level < 0) {
		// failed to read the actual value, fallback to cached value
		ucontrol->value.enumerated.item[0] = cfg->spksw_level;
	} else {
...
static int fsm_spksw_gpio_put(struct snd_kcontrol *kcontrol,
			struct snd_ctl_elem_value *ucontrol)
{
	fsm_config_t *cfg = fsm_get_config();
	int spksw_level = ucontrol->value.enumerated.item[0];

	pr_err("spksw = %d\n", spksw_level);

#ifdef CONFIG_TARGET_PRODUCT_ULTRASOUND
	if (cfg->ultrasound_on) {
		pr_err("fsm_spksw_gpio_put : ultrasound is on 1! \n");
		fsm_set_volume(0);
		fsm_delay_ms(50);
		pr_err("fsm_spksw_gpio_put : after set volume 0\n");
	}
#endif
	gpio_set_value_cansleep(cfg->spksw_gpio, !!spksw_level);

#ifdef CONFIG_TARGET_PRODUCT_ULTRASOUND
	if ((cfg->ultrasound_on) && (cfg->volume == 0)) {
		pr_err("fsm_spksw_gpio_put : ultrasound is on 2! \n");
		fsm_delay_ms(50);
		fsm_set_volume(255);
		pr_err("fsm_spksw_gpio_put : after set volume 255\n");
	}
#endif
...
static const struct snd_kcontrol_new fsm_snd_controls[] =
{
	...
	SOC_SINGLE_EXT("FSM_AMP_ON", SND_SOC_NOPM, 0, 1, 0,
			fsm_amp_get, fsm_amp_put),
	SOC_SINGLE_EXT("FSM_Rotation", SND_SOC_NOPM, 0, 360, 0,
			fsm_rotation_get, fsm_rotation_put),
	SOC_SINGLE_EXT("FSM_Ultrasound_Enable", SND_SOC_NOPM, 0, 1, 0,
			fsm_ultrasound_get, fsm_ultrasound_put),
	SOC_SINGLE_EXT("FSM_SW_GPIO", SND_SOC_NOPM, 0, 0x3, 0,
			fsm_spksw_gpio_get, fsm_spksw_gpio_put),
#if defined (CONFIG_TARGET_PRODUCT_AMETHYST)
...
int fsm_codec_register(struct device *dev, int id)
{
	...
	if (ret < 0) {
		...
		fs_mievent_report(906001352,"PA detection exception",dev);
#endif
		return ret;
	}
...
#if defined(CONFIG_TARGET_PRODUCT_PIANO) || defined(CONFIG_TARGET_PRODUCT_YUPEI)
	set_smartpa_codec(dev, cfg->codec_dai_name[id]);
#endif
