...
static struct wm_coeff_ctl *wm_adsp_get_ctl(struct wm_adsp *dsp,
					     ...
					     unsigned int alg)
{
	struct wm_coeff_ctl *pos = NULL, *rslt = NULL;
...
{
	struct wm_adsp_alg_region *alg_region = NULL;
			...
			struct snd_kcontrol *kcontrol, int event)
{
	...
	struct wm_adsp *dsp = &dsps[w->shift];
	struct wm_coeff_ctl *ctl;
	adsp_dbg(dsp, "cs35l43_wm_adsp_early_event(%d) start\n", event);
	...
	adsp_dbg(dsp, "cs35l43_wm_adsp_early_event(%d) end\n", event);
...
{
	struct wm_adsp_compr *compr, *tmp = NULL;
...
{
	struct wm_adsp_compr_buf *buf = NULL;
	struct wm_adsp_compr *compr;
	int ret = 0;
