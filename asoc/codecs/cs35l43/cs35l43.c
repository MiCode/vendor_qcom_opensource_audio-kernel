				...
				bool read)
{
	...
	struct cs35l43_write_seq_elem *write_seq_elem = NULL;

	buf = kzalloc(sizeof(u32) * sequence->length, GFP_KERNEL);
	if (!buf) {
		dev_err(cs35l43->dev, "[TF-STABILITY] %s: failed to alloc write seq\n",
			__func__);
		return -ENOMEM;
	}
	...
	if (ret != 0) {
		dev_err(dev, "[TF-STABILITY] %s: Failed to read control\n", __func__);
		goto exit;
	}
...
static int cs35l43_dsp_reset(struct cs35l43_private *cs35l43)
{
	...
	if (retry < 0)
		dev_err(cs35l43->dev, "[TF-STABILITY] %s: Cold boot failed\n", __func__);
...
#ifndef CONFIG_ASYNC_LOAD_FIRMWARE
...
#ifdef CONFIG_ASYNC_LOAD_FIRMWARE
...
static int cs35l43_check_dsp_regs(struct cs35l43_private *cs35l43)
{
	...
	val = be32_to_cpu(val);
	if (val != 2) {
		dev_err(cs35l43->dev, "[TF-STABILITY] %s: Error HALO_STATE = %u\n", __func__, val);
		return -EINVAL;
	}
	...
	val = be32_to_cpu(val);
	if (val != 0) {
		dev_err(cs35l43->dev, "[TF-STABILITY] %s: Error AUDIO_SYSTEM ERROR = %u\n", __func__, val);
		return -EINVAL;
	}
	...
	if (val)
		ret = -EINVAL;
	if (ret) {
		dev_err(cs35l43->dev, "[TF-STABILITY] %s: Error DSP SCRATCH\n", __func__);
		return ret;
	}
...
static irqreturn_t cs35l43_irq(int irq, void *data)
{
	...
	if (status[2] & CS35L43_DSP1_STRM_ARB_ERR_EINT1_MASK) {
		...
		if (cs35l43->error_reason != NULL && err_need_report > 0) {
			dev_err(cs35l43->dev, "[TF-STABILITY] Stream Arb Error type:%d\n", err_need_report);
			if (!mutex_is_locked(&cs35l43->xlog_lock)) {
				 ...
				 struct snd_soc_dai *dai)
{
	...
	if (i < ARRAY_SIZE(cs35l43_fs_rates) &&
		...
		dev_err(cs35l43->dev, "[TF-STABILITY] %s: Unsupported rate\n", __func__);
		return -EINVAL;
	}
				...
				struct cs35l43_platform_data *pdata)
{
	...
	if (ret < 0) {
		dev_err(cs35l43->dev, "[TF-STABILITY] %s: Register codec failed\n", __func__);
		goto err_pm;
	}
	if (cs35l43->low_pwr_mode == CS35L43_LOW_PWR_MODE_HIBERNATE)
		pm_runtime_put_autosuspend(cs35l43->dev);
