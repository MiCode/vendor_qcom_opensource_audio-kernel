...
#define ERR_REG_MAX_LEN 48
#endif
// Memory validation error which indicates that
// dsp is unable to synchronize data frames received on the ASP channels.
#define STRM_ARB_FAILED_SYNC 0x6000004
...
#if defined(CONFIG_TARGET_PRODUCT_HAOTIAN) || defined(CONFIG_TARGET_PRODUCT_BIXI)
#define TDM_MAX_SLOTS 4
#else
#define TDM_MAX_SLOTS 2
#endif
...
#if defined(CONFIG_TARGET_PRODUCT_HAOTIAN) || defined(CONFIG_TARGET_PRODUCT_DADA) || defined(CONFIG_TARGET_PRODUCT_NIRVANA)
...
static const char * const delta_enum_text[] = {"null", "music", "music-fold", "voice", "voice-fold",
                                               "movie", "movie-fold", "voip",  "voip-fold", "video",
                                               "video-fold", "game", "game-fold", "handsfree",
                                               "handsfree-fold", "handset", "handset-fold"};
...
#if IS_ENABLED(CONFIG_MIEV)
		if (!strcmp(regs[i].name, "ERROR") && reg != cs35l43->strm_arb_err_regval_prev) {
			ret = -1;
			cs35l43->strm_arb_err_regval_prev = reg;
			snprintf(cs35l43->error_reg, ERR_REG_MAX_LEN, "[ERROR]");
			dev_err(cs35l43->dev, "stream arb err reg:[ERROR]");
		} else if (!strcmp(regs[i].name, "INVALID_STRM_CNT") && reg != cs35l43->strm_arb_inval_strm_regval_prev) {
			ret = -1;
			cs35l43->strm_arb_inval_strm_regval_prev = reg;
			snprintf(cs35l43->error_reg, ERR_REG_MAX_LEN, "[INVALID_STRM_CNT]");
			dev_err(cs35l43->dev, "stream arb err reg: [INVALID_STRM_CNT]");
		} else if (!strcmp(regs[i].name, "ACT_BOOT_CNT") && reg != cs35l43->strm_arb_act_boot_regval_prev) {
			ret = -1;
			cs35l43->strm_arb_act_boot_regval_prev = reg;
			snprintf(cs35l43->error_reg, ERR_REG_MAX_LEN, "[ACT_BOOT_CNT]");
			dev_err(cs35l43->dev, "stream arb err reg: [ACT_BOOT_CNT]");
		}
#endif
	}
...
static irqreturn_t cs35l43_irq(int irq, void *data)
{
...
#if IS_ENABLED(CONFIG_MIEV)
		mievent_report(906001352,"PA detection exception",cs35l43->dev,"NULL");
#endif
		regmap_write(cs35l43->regmap, CS35L43_IRQ1_EINT_1,
					CS35L43_AMP_ERR_EINT1_MASK);
	...
	if (status[2] & CS35L43_DSP1_NMI_ERR_EINT1_MASK) {
		...
		cs35l43_log_dsp_err(cs35l43);
#if IS_ENABLED(CONFIG_MIEV)
		mievent_report(906001355,"Cirrus PA dsp NMI Error",cs35l43->dev,"NULL");
#endif
		if (!mutex_is_locked(&cs35l43->err_lock))
			queue_work(cs35l43->err_wq, &cs35l43->err_work);
	}
	...
	if (status[2] & CS35L43_DSP1_MPU_ERR_EINT1_MASK) {
		...
		cs35l43_log_dsp_err(cs35l43);
#if IS_ENABLED(CONFIG_MIEV)
		mievent_report(906001355,"Cirrus PA dsp MPU Error",cs35l43->dev,"NULL");
#endif
		if (!mutex_is_locked(&cs35l43->err_lock))
			queue_work(cs35l43->err_wq, &cs35l43->err_work);
	}
	...
	if (status[2] & CS35L43_DSP1_STRM_ARB_ERR_EINT1_MASK) {
		...
		if (err_need_report && cs35l43->error_reg != NULL) {
			dev_info(cs35l43->dev, "[TF-STABILITY]Stream Arb Error type:%d, Error reg:%s ", err_need_report, cs35l43->error_reg);
			mievent_report(906001355,"Cirrus PA dsp Stream Arb Error",cs35l43->dev,cs35l43->error_reg);
		}
#else
		cs35l43_log_dsp_err(cs35l43);
#endif
...
static int cs35l43_dsp_init(struct cs35l43_private *cs35l43)
{
	...
	if (ret != 0) {
		dev_err(cs35l43->dev, "[TF-STABILITY] wm_halo_init failed\n");
		goto err;
	}
...
#if IS_ENABLED(CONFIG_MIEV)
	cs35l43->strm_arb_err_regval_prev = 0;
	cs35l43->strm_arb_inval_strm_regval_prev = 0;
	cs35l43->strm_arb_act_boot_regval_prev = 0;

	cs35l43->error_reg = devm_kzalloc(cs35l43->dev, ERR_REG_MAX_LEN, GFP_KERNEL);
	if (!cs35l43->error_reg) {
		dev_err(cs35l43->dev, "%s: no mem for miev error reason\n", __func__);
	}
#endif
...
err_mem:
#if IS_ENABLED(CONFIG_MIEV)
	if (cs35l43->error_reg != NULL) {
		devm_kfree(cs35l43->dev, cs35l43->error_reg);
	}
#endif
err:
probe_err_count++;
...
if (probe_err_count >= probe_max_count) {
	dev_warn(cs35l43->dev, "%s: Register dummy codec [TF-NOSOUND]\n", __func__);
#if IS_ENABLED(CONFIG_MIEV)
	mievent_report(906001351,"PA internal exception",cs35l43->dev,"NULL");
#endif
...
int mievent_report(unsigned int eventid,const char *value,struct device *dev, const char *reg)
{
#if IS_ENABLED(CONFIG_MIEV)
	...
	if ( 906001355 != eventid) {
		cdev_tevent_add_str(mievent, "Keyword", value);
	} else {
		cdev_tevent_add_str(mievent, "ErrReason", value);
		cdev_tevent_add_str(mievent, "ErrRegister", reg);
	}
