...
static bool wcd_is_special_headset(struct wcd_mbhc *mbhc)
{
	...
	if (mbhc->mbhc_cb->mbhc_micbias_control &&
		...
		ret = mbhc->mbhc_cb->mbhc_micb_ctrl_thr_mic(mbhc->component,
							MIC_BIAS_2, true);
		if (ret) {
			pr_err("%s: mbhc_micb_ctrl_thr_mic failed, ret: %d [TF-HEADSET]\n",
				__func__, ret);
#if IS_ENABLED(CONFIG_MIEV)
			mievent  = cdev_tevent_alloc(906005002);
		...
		if ((output_mv <= hs_threshold) &&
			...
			if (ret < 0)
				...
				if (pt_gnd_mic_swap_cnt <
					 ...
					 */
					pr_debug("%s: switch did not work [TF-HEADSET]\n",
						 __func__);
					plug_type = MBHC_PLUG_TYPE_GND_MIC_SWAP;
					goto report;
				} else {
...
#if IS_ENABLED(CONFIG_MIEV)
	if (report_plug == 0) {
		pr_err("%s: Not reporting plug [TF-HEADSET]\n", __func__);
		mievent  = cdev_tevent_alloc(906005003);
		cdev_tevent_add_int(mievent, "MbhcStage", MBHC_DETECT_PLUG);
		cdev_tevent_write(mievent);
		cdev_tevent_destroy(mievent);
	}
