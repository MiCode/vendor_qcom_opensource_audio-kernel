...
static int cs35l41_passport_put(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component =
		snd_soc_kcontrol_component(kcontrol);
	...
	if (cs35l41->dsp.booted) {
		cs35l41->passport = ucontrol->value.integer.value[0];
		if (cs35l41->passport)
			val = 0x400000;
		else
			val = 0;
		...
		val = cpu_to_be32(val);
		ret = wm_adsp_write_ctl(&cs35l41->dsp, "REDUCE_POWER", WMFW_ADSP2_XM,
				0xcd, (void *)&val, sizeof(__be32));
		if (ret < 0) {
			dev_err(cs35l41->dev,
					"Failed to write 'REDUCE_POWER', ret = %d\n", ret);
		}
	}

	dev_dbg(cs35l41->dev, "passport = %x\n", val);

	return 0;
}

static int cs35l41_passport_get(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component = snd_soc_kcontrol_component(kcontrol);
	struct cs35l41_private	*cs35l41 = snd_soc_component_get_drvdata(component);

	ucontrol->value.enumerated.item[0] = cs35l41->passport;
