...
{
	struct frsm_dev *frsm_dev = (struct frsm_dev *)kcontrol->private_value;
	bool state_on;
...
static int frsm_codec_switch_info(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_info *uinfo)
{
	...
	uinfo->count = 1;
	count = ARRAY_SIZE(frsm_codec_switch);
	...
	strlcpy(uinfo->value.enumerated.name,
		frsm_codec_switch[uinfo->value.enumerated.item],
		strlen(frsm_codec_switch[uinfo->value.enumerated.item]) + 1);
		...
		struct snd_ctl_elem_info *uinfo)
{
	...
	frsm_dev = (struct frsm_dev *)kcontrol->private_value;
	if (frsm_dev == NULL || frsm_dev->dev == NULL)
		return -EINVAL;
...
static const struct snd_kcontrol_new frsm_codec_controls[] = {
	{
		...
		.name = "FSAMP Switch",
		.info = frsm_codec_switch_info,
		.get = frsm_codec_switch_get,
		.put = frsm_codec_switch_put,
	},
	{
		.iface = SNDRV_CTL_ELEM_IFACE_MIXER,
		.name = "FSAMP Scene",
		.info = frsm_codec_scene_info,
		.get = frsm_codec_scene_get,
		.put = frsm_codec_scene_put,
	},
};
	...
	struct snd_soc_component *codec)
{
	...
	for (i = 0; i < count; i++) {
		...
		new_kctrl[i].private_value = (unsigned long)frsm_dev;
	}

	return snd_soc_add_component_controls(
		codec, new_kctrl, count);
}

int frsm_add_codec_controls(struct snd_soc_component *codec)
{
	struct list_head *dev_list = NULL;
	struct frsm_dev *frsm_dev = NULL;
	int ret;

	/*get dev list*/
	ret = frsm_get_list_head(&dev_list);
	if (ret) {
		pr_err("list is null\n");
		return ret;
	}

	list_for_each_entry(frsm_dev, dev_list, list) {
		ret = frsm_init_cmpnt_controls(frsm_dev, codec);
		if (ret < 0)
			return ret;
	}
...
EXPORT_SYMBOL(frsm_add_codec_controls);
...
#endif
{
	...
	int ret;
#if IS_ENABLED(CONFIG_LCT_AUDIO_INFO)
	char lct_audio_pa_info[AUDIO_INFO_NAME_MAX] = {'\0'};
#endif
...
int get_frsm_probe_complete_flag(void)
{
	return g_probe_complete_flag;
}
EXPORT_SYMBOL(get_frsm_probe_complete_flag);
