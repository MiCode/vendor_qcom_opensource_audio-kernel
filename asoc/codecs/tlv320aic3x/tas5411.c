...
int tas5411_pinctrl_controller(bool amp_mute_status) {
	...
	if (tas5411_priv->tas5411_mute_pinctrl && tas5411_priv->enable_mute_pin_control) {
		if (amp_mute_status) {
			if (tas5411_priv->pinctrl_state[0]) {
			...
			if (tas5411_priv->pinctrl_state[1]) {
...
static void tas5411_pa_mute_switch_deinit(struct tas5411_data *tas5411)
{
	if(!tas5411) {
		return;
	}
...
int tas5411_pa_boot_reg(struct tas5411_data *tas5411) {
	int ret = 0;
	unsigned int timeout = 0;

	do {
		ret = regmap_write(tas5411_priv->regmap, TAS5411_REG_CONTROL, 0x70);
		if (ret == 0) {
			dev_err(tas5411_priv->dev, "ok write TAS5411_REG_CONTROL, ret = %d", ret);
			break;
		} else if (ret == -ENOTCONN) {
			dev_err(tas5411_priv->dev, "error write TAS5411_REG_CONTROL, ret = %d", ret);
			msleep(100);
			timeout += 100;
			dev_err(tas5411_priv->dev, "every add 100ms sleep, timeout = %d", timeout);
		} else {
			dev_err(tas5411_priv->dev, "other error write TAS5411_REG_CONTROL, ret = %d", ret);
			return ret;
		}
...
int tas5411_probe(struct i2c_client *client,
				const struct i2c_device_id *id)
{
	...
	dev_info(&client->dev, "tas5411 probe E\n");
	...
	ret = tas5411_pa_mute_switch_init(tas5411_priv);
	if (ret < 0) {
		dev_err(tas5411->dev, "error tas5411_pa_mute_switch_init");
		goto err1;
	}
	/* 14-V peak output */
	ret = tas5411_pa_boot_reg(tas5411_priv);
	if (ret < 0) {
		dev_err(tas5411->dev, "error tas5411_pa_boot_reg");
		goto err2;
	}

	dev_info(&client->dev, "tas5411 probe X, ret %d\n",  ret);
	return 0;
err2:
	tas5411_pa_mute_switch_deinit(tas5411_priv);
err1:
...
EXPORT_SYMBOL(tas5411_probe);

void tas5411_remove(struct i2c_client *client)
{
	if(!client)
		return;
