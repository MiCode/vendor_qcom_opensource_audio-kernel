...
static int aic3x_init(struct snd_soc_component *component)
{
	...
	/* mute line outputs */
	snd_soc_component_update_bits(component, LLOPM_CTRL, LINE_OUT_MUTE, LINE_OUT_MUTE);
	snd_soc_component_update_bits(component, RLOPM_CTRL, LINE_OUT_MUTE, LINE_OUT_MUTE);
	/* unmute all outputs */
...
int aic3x_suspend(struct device *dev)
{
	dev_info(dev, "%s, do nothing\n", __func__);
	return 0;
}

int aic3x_resume(struct device *dev)
{
	struct aic3x_priv *aic3x = dev_get_drvdata(dev);
	dev_info(dev, "%s\n", __func__);

	if (aic3x->aic3x_reset_pinctrl && aic3x->enable_reset_pin_control) {
		if (aic3x->pinctrl_state[0] && aic3x->pinctrl_state[1]) {
			pinctrl_select_state(aic3x->aic3x_reset_pinctrl, aic3x->pinctrl_state[1]);
			dev_info(dev, "%s: reset_gpio_output_status %d, rcv switch gpio pull down\n", __func__,
					aic3x->reset_gpio_output_status);
			msleep(5);
			pinctrl_select_state(aic3x->aic3x_reset_pinctrl, aic3x->pinctrl_state[0]);
			dev_info(dev, "%s: reset_gpio_output_status %d, rcv switch gpio pull up\n", __func__,
					aic3x->reset_gpio_output_status);
			aic3x->reset_gpio_output_status = true;
		}
	}
...
err_gpio:
	...
	// probe fail, reset_pin not enable
	aic3x->enable_reset_pin_control = 0;
	aic3x->aic3x_reset_pinctrl = NULL;
err:
	probe_err_count++;
