...
static int tas25xx_rcv_handset_switch_init(struct tas25xx_priv *p_tas25xx)
{
	int ret = 0;
	int i = 0;
	p_tas25xx->enable_rcv_pin_control = 0;
	p_tas25xx->tas25xx_rcv_pinctrl = devm_pinctrl_get(p_tas25xx->platform_data->dev);
	if (!(p_tas25xx->tas25xx_rcv_pinctrl)) {
 		dev_err(p_tas25xx->platform_data->dev, "Target does not use pinctrl\n");
 		p_tas25xx->tas25xx_rcv_pinctrl = NULL;
 		ret = -EINVAL;
 		return ret;
 	}
 	for (i = 0; i < ARRAY_SIZE(rcv_pctl_names); i++) {
 		const char *n = rcv_pctl_names[i];
 		struct pinctrl_state *state = pinctrl_lookup_state(p_tas25xx->tas25xx_rcv_pinctrl, n);
 		if (!(state)) {
 			dev_err(p_tas25xx->platform_data->dev, "cannot find '%s'\n", n);
 			ret = -EINVAL;
 			return ret;
 		}
 		dev_info(p_tas25xx->platform_data->dev, "found pin control %s\n", n);
 		p_tas25xx->pinctrl_state[i] = state;
 		p_tas25xx->enable_rcv_pin_control = 1;
 	}
	return ret;
}
...
static int tas25xx_i2c_probe(struct i2c_client *p_client)
{
	...
	ret = tas25xx_rcv_handset_switch_init(p_tas25xx);
	if(ret){
		dev_err(&p_client->dev,"tas25xx_rcv_handset_switch_init failed \n");
	}
