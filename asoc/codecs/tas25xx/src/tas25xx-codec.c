...
static const struct snd_soc_dapm_route tas25xx_audio_map[] = {
 ...
 static int tas25xx_rcv_switch_pinctrl_get(struct snd_kcontrol *kcontrol,struct snd_ctl_elem_value *ucontrol)
 {
 	struct snd_soc_component *component = snd_soc_kcontrol_component(kcontrol);
 	struct tas25xx_priv *p_tas25xx = snd_soc_component_get_drvdata(component);
 	ucontrol->value.integer.value[0] = 0;
 	if (p_tas25xx->tas25xx_rcv_pinctrl && p_tas25xx->enable_rcv_pin_control)
 		ucontrol->value.integer.value[0] = p_tas25xx->enable_rcv_switch;
 	return 0;
 }

 static int tas25xx_rcv_switch_pinctrl_put(struct snd_kcontrol *kcontrol,struct snd_ctl_elem_value *ucontrol)
 {
     struct snd_soc_component *component = snd_soc_kcontrol_component(kcontrol);
     struct tas25xx_priv *p_tas25xx = snd_soc_component_get_drvdata(component);

     p_tas25xx->enable_rcv_switch = ucontrol->value.integer.value[0];

     if (p_tas25xx->tas25xx_rcv_pinctrl && p_tas25xx->enable_rcv_pin_control) {
         if (p_tas25xx->enable_rcv_switch) {
             dev_info(p_tas25xx->platform_data->dev, "%s: enable_rcv_switch %d, rcv switch gpio pull up\n", __func__,p_tas25xx->enable_rcv_switch);
             pinctrl_select_state(p_tas25xx->tas25xx_rcv_pinctrl,p_tas25xx->pinctrl_state[0]);
         } else {
             dev_info(p_tas25xx->platform_data->dev, "%s: enable_rcv_switch %d, rcv switch gpio pull down\n", __func__,p_tas25xx->enable_rcv_switch);
             pinctrl_select_state(p_tas25xx->tas25xx_rcv_pinctrl,p_tas25xx->pinctrl_state[1]);
         }
     }
     return 0;
 }

static const struct snd_kcontrol_new tas25xx_switch_controls[] = {
	SOC_SINGLE_EXT("Enable RCV Pin Control", SND_SOC_NOPM, 0, 1, 0,tas25xx_rcv_switch_pinctrl_get, tas25xx_rcv_switch_pinctrl_put),
};
...
static struct snd_soc_component_driver soc_codec_driver_tas25xx = {
	...
	.controls = tas25xx_switch_controls,
	.num_controls = ARRAY_SIZE(tas25xx_switch_controls),
};
