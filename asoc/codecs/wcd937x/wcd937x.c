...
#define PA_DEV 2
static DEFINE_MUTEX(g_pa_status_mutex);
struct pa_status {
	u8 enable;
	u8 mode;
} g_pa_status[PA_DEV] = {{0, 0}, {0, 0}};

enum wcd937x_dev_scene {
	WCD937X_SCENE_MUSIC = 0,
	WCD937X_SCENE_VOICE,
	WCD937X_SCENE_VOIP,
	WCD937X_SCENE_FM,
	WCD937X_SCENE_EAR,
	WCD937X_SCENE_BYPASS,
	WCD937X_SCENE_OFF,
	WCD937X_SCENE_INVARIABLE,
	WCD937X_SCENE_MAX
};

static const char *const wcd937x_ext_spk_mode_function[] = {
	"Music", "Voice", "Voip", "Fm", "Receiver", "Bypass", "Off", "Invariable",
};
...
static char const *amp_switch_text[] = {"Off", "On"};
static const struct soc_enum amp_switch_enum = SOC_ENUM_SINGLE_EXT(
	ARRAY_SIZE(amp_switch_text), amp_switch_text);
static atomic_t amp_switch_top;
static atomic_t amp_switch_bot;
	...
	case SND_SOC_DAPM_POST_PMD:
		...
		if (wcd937x->ear_rx_path & EAR_RX_PATH_AUX)
			...
			snd_soc_component_update_bits(component,
					...
					0x07, 0x00);
		//ADD: Audio_OCP_Enable
		//case 05657117 05669963 CR-3062852 BEGAIN
		usleep_range(10000, 10100);
		/* disable EAR CnP FSM */
		snd_soc_component_update_bits(component, WCD937X_EAR_EAR_EN_REG, 0x02, 0x00);
		/* toggle EAR PA to let PA control registers take effect */
		snd_soc_component_update_bits(component, WCD937X_ANA_EAR, 0x80, 0x80);
		snd_soc_component_update_bits(component, WCD937X_ANA_EAR, 0x80, 0x00);
		/* enable EAR CnP FSM */
		snd_soc_component_update_bits(component, WCD937X_EAR_EAR_EN_REG, 0x02, 0x02);
		//case 05657117 05669963 CR-3062852 END
		//END Audio_OCP_Enable
		break;
	};
	return ret;
...
#if IS_ENABLED(CONFIG_SND_SOC_SIPA)
static int sipa_dev_pa(bool enable, int mode, int pa_idx)
{
	int ret = 0;

	mutex_lock(&g_pa_status_mutex);
	if(enable){
		//Prevent duplicate settings when turnon pa
		if (pa_idx > 0 && pa_idx <= PA_DEV) {
			pr_info("%s: pa_idx %d enable %d mode %d before", __func__, pa_idx,
				g_pa_status[pa_idx - 1].enable, g_pa_status[pa_idx - 1].mode);
			pr_info("%s: pa_idx %d enable %d mode %d new", __func__, pa_idx,
				enable, mode);
			if (g_pa_status[pa_idx - 1].enable == enable &&
				g_pa_status[pa_idx - 1].mode == mode) {
				goto sipa_exit;
			}
		}

		switch (mode) {
			case WCD937X_SCENE_MUSIC:
				sipa_multi_channel_power_on_and_set_scene(SIPA_SCENE_PLAYBACK, pa_idx);
				break;
			case WCD937X_SCENE_VOICE:
				sipa_multi_channel_power_on_and_set_scene(SIPA_SCENE_VOICE, pa_idx);
				break;
			case WCD937X_SCENE_VOIP:
				sipa_multi_channel_power_on_and_set_scene(SIPA_SCENE_VOIP, pa_idx);
				break;
			case WCD937X_SCENE_BYPASS:
				sipa_multi_channel_power_on_and_set_scene(SIPA_SCENE_FACTORY, pa_idx);
				break;
			case WCD937X_SCENE_EAR:
				sipa_multi_channel_power_on_and_set_scene(SIPA_SCENE_RECEIVER, pa_idx);
				break;
			case WCD937X_SCENE_FM:
				sipa_multi_channel_power_on_and_set_scene(SIPA_SCENE_PLAYBACK, pa_idx);
				break;
			default:
				sipa_multi_channel_power_on_and_set_scene(SIPA_SCENE_PLAYBACK, pa_idx);
		...
		sipa_multi_channel_power_off(pa_idx);
	}

sipa_exit:
	g_pa_status[pa_idx - 1].enable = enable;
	g_pa_status[pa_idx - 1].mode = mode;
	mutex_unlock(&g_pa_status_mutex);

	if (pa_idx == SIPA_DEV_L)
		atomic_set(&amp_switch_top, !!enable);
	else if (pa_idx == SIPA_DEV_R)
		atomic_set(&amp_switch_bot, !!enable);
...
#if IS_ENABLED(CONFIG_SND_SOC_FS181X)
static int fsm_dev_pa(bool enable, int mode, int pa_idx)
{
	int ret = 0;
	...
	if(enable){
		...
		switch (mode) {
			case WCD937X_SCENE_MUSIC:
				frsm_i2ca_set_scene(pa_idx, FSM_SCENE_MUSIC);
				break;
			case WCD937X_SCENE_VOICE:
				frsm_i2ca_set_scene(pa_idx, FSM_SCENE_VOICE);
				break;
			case WCD937X_SCENE_VOIP:
				frsm_i2ca_set_scene(pa_idx, FSM_SCENE_VOICE);
				break;
			case WCD937X_SCENE_BYPASS:
				frsm_i2ca_set_scene(pa_idx, FSM_SCENE_BYPASS);
				break;
			case WCD937X_SCENE_EAR:
				frsm_i2ca_set_scene(pa_idx, FSM_SCENE_RCV);
				break;
			case WCD937X_SCENE_FM:
				frsm_i2ca_set_scene(pa_idx, FSM_SCENE_MUSIC);
				break;
	...
	if(ret == 0){
		frsm_i2ca_spk_switch(pa_idx, enable);
	}
...
#if IS_ENABLED(CONFIG_WCD937X_EXT_SPK)
static unsigned int g_wcd937x_ear_ext_spk_mode = WCD937X_SCENE_OFF;
static unsigned int g_wcd937x_aux_ext_spk_mode = WCD937X_SCENE_OFF;
static int wcd937x_ear_ext_spk_mode_get(struct snd_kcontrol *kcontrol,
								struct snd_ctl_elem_value *ucontrol)
{
	int current_mode = g_wcd937x_ear_ext_spk_mode;
	ucontrol->value.integer.value[0] = current_mode;
	pr_debug("%s: get mode:%d\n", __func__, current_mode);
	return 0;
}

static int wcd937x_ear_ext_spk_mode_set(struct snd_kcontrol *kcontrol,
								struct snd_ctl_elem_value *ucontrol)
{
	int set_mode;
	int current_mode = WCD937X_SCENE_OFF;
	...
	if (set_mode == WCD937X_SCENE_OFF || set_mode == WCD937X_SCENE_INVARIABLE) {
		pr_debug("%s: ignore set mode:%d\n", __func__, set_mode);
		return 0;
	} else if (set_mode < WCD937X_SCENE_MUSIC || set_mode >= WCD937X_SCENE_MAX) {
		pr_err("%s: Invalid set mode:%d\n", __func__, set_mode);
		return -EINVAL;
	}
	current_mode = g_wcd937x_ear_ext_spk_mode;
	if ((current_mode != WCD937X_SCENE_OFF) && (current_mode != set_mode)) {
		#if IS_ENABLED(CONFIG_LCT_AUDIO_INFO)
		if (g_wcd937x_ext_pa_info == PA_CHIP_TYPE_SIPA) {
			#if IS_ENABLED(CONFIG_SND_SOC_SIPA)
			sipa_dev_pa(true, set_mode, SIPA_DEV_L);
			#endif
		} else if (g_wcd937x_ext_pa_info == PA_CHIP_TYPE_FSM) {	
			#if IS_ENABLED(CONFIG_SND_SOC_FS181X)
			fsm_dev_pa(true, set_mode, FSM_DEV_L);
			#endif
		}
		#endif
	}
	g_wcd937x_ear_ext_spk_mode = set_mode;
                ...
                struct snd_ctl_elem_value *ucontrol)
{
	...
	if ((current_mode != WCD937X_SCENE_OFF) && (current_mode != set_mode)) {
		...
		if (g_wcd937x_ext_pa_info == PA_CHIP_TYPE_SIPA) {
			#if IS_ENABLED(CONFIG_SND_SOC_SIPA)
			sipa_dev_pa(true, set_mode, SIPA_DEV_R);
			#endif
		} else if (g_wcd937x_ext_pa_info == PA_CHIP_TYPE_FSM) {
			#if IS_ENABLED(CONFIG_SND_SOC_FS181X)
			fsm_dev_pa(true, set_mode, FSM_DEV_R);
			#endif
		}
		#endif
	}
...
static int amp_switch_top_set(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	...
	if (g_wcd937x_ext_pa_info == PA_CHIP_TYPE_SIPA) {
		#if IS_ENABLED(CONFIG_SND_SOC_SIPA)
		sipa_dev_pa(enable, g_wcd937x_ear_ext_spk_mode, SIPA_DEV_L);
		#endif
	} else if (g_wcd937x_ext_pa_info == PA_CHIP_TYPE_FSM) {
		#if IS_ENABLED(CONFIG_SND_SOC_FS181X)
		fsm_dev_pa(enable, g_wcd937x_ear_ext_spk_mode, FSM_DEV_L);
		#endif
	}
	#endif
...
static int amp_switch_top_get(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	int enable;

	enable = atomic_read(&amp_switch_top);
	ucontrol->value.integer.value[0] = enable;

	return 0;
}

static int amp_switch_bot_set(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	int enable = ucontrol->value.integer.value[0];
	...
	if (g_wcd937x_ext_pa_info == PA_CHIP_TYPE_SIPA) {
		#if IS_ENABLED(CONFIG_SND_SOC_SIPA)
		sipa_dev_pa(enable, g_wcd937x_aux_ext_spk_mode, SIPA_DEV_R);
		#endif
	} else if (g_wcd937x_ext_pa_info == PA_CHIP_TYPE_FSM) {
		#if IS_ENABLED(CONFIG_SND_SOC_FS181X)
		fsm_dev_pa(enable, g_wcd937x_aux_ext_spk_mode, FSM_DEV_R);
		#endif
	}
	#endif
...
static int amp_switch_bot_get(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	int enable;

	enable = atomic_read(&amp_switch_bot);
	ucontrol->value.integer.value[0] = enable;
...
static const struct snd_kcontrol_new wcd937x_snd_controls[] = {
	...
	//END AUX_PATH_MODE
#if IS_ENABLED(CONFIG_WCD937X_EXT_SPK)
	SOC_ENUM_EXT("EAR EXT SPK MODE", wcd937x_ext_spk_mode_enum,
		wcd937x_ear_ext_spk_mode_get, wcd937x_ear_ext_spk_mode_set),
	SOC_ENUM_EXT("AUX EXT SPK MODE", wcd937x_ext_spk_mode_enum,
		wcd937x_aux_ext_spk_mode_get, wcd937x_aux_ext_spk_mode_set),
	SOC_ENUM_EXT("AMP_SWITCH_TOP", amp_switch_enum,
		amp_switch_top_get, amp_switch_top_set),
	SOC_ENUM_EXT("AMP_SWITCH_BOT", amp_switch_enum,
		amp_switch_bot_get, amp_switch_bot_set),
#endif /* CONFIG_WCD937X_EXT_SPK */
};
...
#if IS_ENABLED(CONFIG_WCD937X_EXT_SPK)
static int ear_ext_spk_event(struct snd_soc_dapm_widget *w,
					struct snd_kcontrol *control, int event)
{
	int mode = g_wcd937x_ear_ext_spk_mode;
	#if IS_ENABLED(CONFIG_LCT_AUDIO_INFO)
	g_wcd937x_ext_pa_info = audio_pa_chip_type_get();
	...
	switch (event) {
		case SND_SOC_DAPM_POST_PMU:
			...
			if (g_wcd937x_ext_pa_info == PA_CHIP_TYPE_SIPA) {
				#if IS_ENABLED(CONFIG_SND_SOC_SIPA)
				sipa_dev_pa(true, mode, SIPA_DEV_L);
				#endif
			} else if (g_wcd937x_ext_pa_info == PA_CHIP_TYPE_FSM) {	
				#if IS_ENABLED(CONFIG_SND_SOC_FS181X)
				fsm_dev_pa(true, mode, FSM_DEV_L);
				#endif
			}
			#endif
			break;
		case SND_SOC_DAPM_PRE_PMD:
			...
			if (g_wcd937x_ext_pa_info == PA_CHIP_TYPE_SIPA) {
				#if IS_ENABLED(CONFIG_SND_SOC_SIPA)
				sipa_dev_pa(false, mode, SIPA_DEV_L);
				#endif
			} else if (g_wcd937x_ext_pa_info == PA_CHIP_TYPE_FSM) {	
				#if IS_ENABLED(CONFIG_SND_SOC_FS181X)
				fsm_dev_pa(false, mode, FSM_DEV_L);
				#endif
			}
			#endif
			break;
					...
					struct snd_kcontrol *control, int event)
{
	...
	switch (event) {
		case SND_SOC_DAPM_POST_PMU:
			...
			if (g_wcd937x_ext_pa_info == PA_CHIP_TYPE_SIPA) {
				#if IS_ENABLED(CONFIG_SND_SOC_SIPA)
				sipa_dev_pa(true, mode, SIPA_DEV_R);
				#endif
			} else if (g_wcd937x_ext_pa_info == PA_CHIP_TYPE_FSM) {	
				#if IS_ENABLED(CONFIG_SND_SOC_FS181X)
				fsm_dev_pa(true, mode, FSM_DEV_R);
				#endif
			}
			#endif
			break;
		case SND_SOC_DAPM_PRE_PMD:
			...
			if (g_wcd937x_ext_pa_info == PA_CHIP_TYPE_SIPA) {
				#if IS_ENABLED(CONFIG_SND_SOC_SIPA)
				sipa_dev_pa(false, mode, SIPA_DEV_R);
				#endif
			} else if (g_wcd937x_ext_pa_info == PA_CHIP_TYPE_FSM) {	
				#if IS_ENABLED(CONFIG_SND_SOC_FS181X)
				fsm_dev_pa(false, mode, FSM_DEV_R);
				#endif
			}
			#endif
			break;
...
static const struct snd_soc_dapm_widget wcd937x_dapm_widgets[] = {
	...
	SND_SOC_DAPM_OUT_DRV_E("EAR EXT SPK", SND_SOC_NOPM, 0, 0, NULL, 0,
				ear_ext_spk_event, SND_SOC_DAPM_POST_PMU |
				SND_SOC_DAPM_PRE_PMD),
#endif /* CONFIG_WCD937X_EXT_SPK */
...
#if IS_ENABLED(CONFIG_WCD937X_EXT_SPK)
	{"EAR EXT SPK", NULL, "EAR PGA"},
	{"EAR", NULL, "EAR EXT SPK"},
#else
	{"EAR", NULL, "EAR PGA"},
#endif /* CONFIG_WCD937X_EXT_SPK */
};
