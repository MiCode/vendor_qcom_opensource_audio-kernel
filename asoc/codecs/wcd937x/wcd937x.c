...
/* same as aw_acf_bin.h */
enum wcd937x_dev_profile_id {
	WCD937X_PROFILE_MUSIC = 0x0000,
	WCD937X_PROFILE_VOICE,
	WCD937X_PROFILE_VOIP,
	WCD937X_PROFILE_RINGTONE,
	WCD937X_PROFILE_RINGTONE_HS,
	WCD937X_PROFILE_LOWPOWER,
	WCD937X_PROFILE_BYPASS,
	WCD937X_PROFILE_MMI,
	WCD937X_PROFILE_FM,
	WCD937X_PROFILE_NOTIFICATION,
	WCD937X_PROFILE_RECEIVER,
	WCD937X_PROFILE_OFF,
	WCD937X_PROFILE_INVARIABLE,
	WCD937X_PROFILE_MAX,
};

static const char *const wcd937x_ext_spk_mode_function[] = {
	"Music", "Voice", "Voip", "Ringtone", "Ringtone_hs", "Lowpower",
	"Bypass", "Mmi", "Fm", "Notification", "Receiver", "Off", "Invariable",
};
	...
	case SND_SOC_DAPM_POST_PMU:
		...
		if (hph_mode == CLS_AB || hph_mode == CLS_AB_HIFI)
			snd_soc_component_update_bits(component,
				WCD937X_ANA_RX_SUPPLIES,
				0x02, 0x02);
		wcd937x->ear_hphl_pga_count++; /* ADD: Audio_Hph_Ear_Aux */
		if (wcd937x->update_wcd_event)
			wcd937x->update_wcd_event(wcd937x->handle,
						SLV_BOLERO_EVT_RX_MUTE,
						(WCD_RX1 << 0x10));
...
#if IS_ENABLED(CONFIG_SND_SOC_AW87XXX)
static int aw87xxx_dev_pa(bool enable, int mode, int dev_index)
{
	int ret = 0;
	int set_mode = AW_PROFILE_OFF;

	if (false == enable)
		set_mode = AW_PROFILE_OFF;
	else
		set_mode = mode;
	pr_info("%s:aw87xxx_dev_id %d, aw87xxx_dev_mode %d\n", __func__ ,dev_index , set_mode);

	ret = aw87xxx_set_profile(dev_index, aw_profile[set_mode]);
	if (ret < 0) {
...
#if IS_ENABLED(CONFIG_SND_SOC_FS181X)
static int fsm_dev_pa(bool enable, int mode, int spkid)
{
	int ret = 0;
	if(enable){
		switch (mode) {
			case WCD937X_PROFILE_MUSIC:
				frsm_i2ca_set_scene(spkid, FSM_SCENE_MUSIC);
				break;
			case WCD937X_PROFILE_VOICE:
				frsm_i2ca_set_scene(spkid, FSM_SCENE_VOICE);
				break;
			case WCD937X_PROFILE_VOIP:
				frsm_i2ca_set_scene(spkid, FSM_SCENE_VOICE);
				break;
			case WCD937X_PROFILE_BYPASS:
				frsm_i2ca_set_scene(spkid, FSM_SCENE_BYPASS);
				break;
			case WCD937X_PROFILE_FM:
				frsm_i2ca_set_scene(spkid, FSM_SCENE_MUSIC);
				break;
			default:
				ret = -EPERM;
	...
	if(ret == 0){
		frsm_i2ca_spk_switch(spkid, enable);
	}
	return ret;
}
#endif /* CONFIG_SND_SOC_FS181X */
...
#if IS_ENABLED(CONFIG_WCD937X_EXT_SPK)
static unsigned int g_wcd937x_aux_ext_spk_mode = WCD937X_PROFILE_OFF;
...
{
	int ret = 0;
	int set_mode = WCD937X_PROFILE_OFF;
	int current_mode = WCD937X_PROFILE_OFF;
	set_mode = ucontrol->value.integer.value[0];
	if (set_mode == WCD937X_PROFILE_INVARIABLE) {
		pr_debug("%s: ignore set mode:%d\n", __func__, set_mode);
		return ret;
	} else if (set_mode < WCD937X_PROFILE_MUSIC || set_mode >= WCD937X_PROFILE_MAX) {
		pr_err("%s: Invalid set mode:%d\n", __func__, set_mode);
		return -EINVAL;
	}
	current_mode = g_wcd937x_aux_ext_spk_mode;
	if ((current_mode != WCD937X_PROFILE_OFF) && (current_mode != set_mode)) {
		#if IS_ENABLED(CONFIG_LCT_AUDIO_INFO)
		g_wcd937x_ext_pa_info = audio_pa_chip_type_get();
		pr_err("%s: painfo:%d, force update mode:%d\n", __func__, g_wcd937x_ext_pa_info, set_mode);
		if (g_wcd937x_ext_pa_info == PA_CHIP_TYPE_AW) {
			#if IS_ENABLED(CONFIG_SND_SOC_AW87XXX)
			ret = aw87xxx_dev_pa(true, set_mode, AW_DEV_0);
			ret = aw87xxx_dev_pa(true, set_mode, AW_DEV_1);
			#endif /* CONFIG_SND_SOC_AW87XXX */
		} else if (g_wcd937x_ext_pa_info == PA_CHIP_TYPE_FSM) {
			#if IS_ENABLED(CONFIG_SND_SOC_FS181X)
			ret = fsm_dev_pa(true, set_mode, FSM_DEV_1);
			ret = fsm_dev_pa(true, set_mode, FSM_DEV_2);
			#endif /* CONFIG_SND_SOC_FS181X */
		}
		#endif /* CONFIG_LCT_AUDIO_INFO */
	}
...
#if IS_ENABLED(CONFIG_WCD937X_AUX_MODE)
...
static const struct snd_kcontrol_new wcd937x_snd_controls[] = {
	...
	SOC_ENUM_EXT("TX CH3 PWR", wcd937x_tx_ch_pwr_level_enum,
		wcd937x_tx_ch_pwr_level_get, wcd937x_tx_ch_pwr_level_put),
#if IS_ENABLED(CONFIG_WCD937X_EXT_SPK)
	SOC_ENUM_EXT("AUX EXT SPK0 MODE", wcd937x_ext_spk_mode_enum,
			wcd937x_aux_ext_spk_mode_get, wcd937x_aux_ext_spk_mode_set),
	SOC_ENUM_EXT("AUX EXT SPK1 MODE", wcd937x_ext_spk_mode_enum,
			wcd937x_aux_ext_spk_mode_get, wcd937x_aux_ext_spk_mode_set),
#endif /* CONFIG_WCD937X_EXT_SPK */
#if IS_ENABLED(CONFIG_WCD937X_AUX_MODE)
...
#if IS_ENABLED(CONFIG_WCD937X_EXT_SPK)
static int aux_ext_spk_0_event(struct snd_soc_dapm_widget *w, struct snd_kcontrol *kcontrol, int event)
{
	int ret = 0;
	...
	switch (event) {
		case SND_SOC_DAPM_POST_PMU:
			pr_info("%s: power on event %d\n", __func__, event);
			#if IS_ENABLED(CONFIG_LCT_AUDIO_INFO)
			if (g_wcd937x_ext_pa_info == PA_CHIP_TYPE_AW) {
				#if IS_ENABLED(CONFIG_SND_SOC_AW87XXX)
				ret = aw87xxx_dev_pa(true, mode, AW_DEV_0);
				#endif /* CONFIG_SND_SOC_AW87XXX */
			} else if (g_wcd937x_ext_pa_info == PA_CHIP_TYPE_FSM) {
				#if IS_ENABLED(CONFIG_SND_SOC_FS181X)
				ret = fsm_dev_pa(true, mode, FSM_DEV_1);
				#endif /* CONFIG_SND_SOC_FS181X */
			}
			#endif /* CONFIG_LCT_AUDIO_INFO */
			break;
		case SND_SOC_DAPM_PRE_PMD:
			pr_info("%s: power off event %d\n", __func__, event);
			#if IS_ENABLED(CONFIG_LCT_AUDIO_INFO)
			if (g_wcd937x_ext_pa_info == PA_CHIP_TYPE_AW) {
				#if IS_ENABLED(CONFIG_SND_SOC_AW87XXX)
				ret = aw87xxx_dev_pa(false, mode, AW_DEV_0);
				#endif /* CONFIG_SND_SOC_AW87XXX */
			} else if (g_wcd937x_ext_pa_info == PA_CHIP_TYPE_FSM) {
				#if IS_ENABLED(CONFIG_SND_SOC_FS181X)
				ret = fsm_dev_pa(false, mode, FSM_DEV_1);
				#endif /* CONFIG_SND_SOC_FS181X */
			}
			#endif /* CONFIG_LCT_AUDIO_INFO */
			g_wcd937x_aux_ext_spk_mode = WCD937X_PROFILE_OFF;
			break;
		default:
...
static int aux_ext_spk_1_event(struct snd_soc_dapm_widget *w, struct snd_kcontrol *kcontrol, int event)
{
	int ret = 0;
	int mode = g_wcd937x_aux_ext_spk_mode;
	#if IS_ENABLED(CONFIG_LCT_AUDIO_INFO)
	...
	switch (event) {
		case SND_SOC_DAPM_POST_PMU:
			...
			if (g_wcd937x_ext_pa_info == PA_CHIP_TYPE_AW) {
				#if IS_ENABLED(CONFIG_SND_SOC_AW87XXX)
				ret = aw87xxx_dev_pa(true, mode, AW_DEV_1);
				#endif /* CONFIG_SND_SOC_AW87XXX */
			} else if (g_wcd937x_ext_pa_info == PA_CHIP_TYPE_FSM) {
				#if IS_ENABLED(CONFIG_SND_SOC_FS181X)
				ret = fsm_dev_pa(true, mode, FSM_DEV_2);
				#endif /* CONFIG_SND_SOC_FS181X */
			}
			#endif /* CONFIG_LCT_AUDIO_INFO */
			break;
		case SND_SOC_DAPM_PRE_PMD:
			...
			if (g_wcd937x_ext_pa_info == PA_CHIP_TYPE_AW) {
				#if IS_ENABLED(CONFIG_SND_SOC_AW87XXX)
				ret = aw87xxx_dev_pa(false, mode, AW_DEV_1);
				#endif /* CONFIG_SND_SOC_AW87XXX */
			} else if (g_wcd937x_ext_pa_info == PA_CHIP_TYPE_FSM) {
				#if IS_ENABLED(CONFIG_SND_SOC_FS181X)
				ret = fsm_dev_pa(false, mode, FSM_DEV_2);
				#endif /* CONFIG_SND_SOC_FS181X */
			}
			#endif /* CONFIG_LCT_AUDIO_INFO */
			g_wcd937x_aux_ext_spk_mode = WCD937X_PROFILE_OFF;
...
static const struct snd_soc_dapm_widget wcd937x_dapm_widgets[] = {
	...
	SND_SOC_DAPM_SUPPLY("VA MIC BIAS3", SND_SOC_NOPM, 0, 0,
				...
				SND_SOC_DAPM_POST_PMD),
#if IS_ENABLED(CONFIG_WCD937X_EXT_SPK)
	SND_SOC_DAPM_OUT_DRV_E("AUX EXT SPK0", SND_SOC_NOPM, 0, 0, NULL, 0,
				aux_ext_spk_0_event, SND_SOC_DAPM_POST_PMU |
				SND_SOC_DAPM_PRE_PMD),
	SND_SOC_DAPM_OUT_DRV_E("AUX EXT SPK1", SND_SOC_NOPM, 0, 0, NULL, 0,
				aux_ext_spk_1_event, SND_SOC_DAPM_POST_PMU |
				SND_SOC_DAPM_PRE_PMD),
#endif /* CONFIG_WCD937X_EXT_SPK */
...
#if IS_ENABLED(CONFIG_WCD937X_EXT_SPK)
	{"AUX EXT SPK0", NULL, "AUX PGA"},
	{"AUX", NULL, "AUX EXT SPK0"},
#else
	{"AUX", NULL, "AUX PGA"},
#endif /* CONFIG_WCD937X_EXT_SPK */
...
#if IS_ENABLED(CONFIG_WCD937X_EXT_SPK)
	{"AUX EXT SPK1", NULL, "EAR PGA"},
	{"EAR", NULL, "AUX EXT SPK1"},
#else
	{"EAR", NULL, "EAR PGA"},
#endif /* CONFIG_WCD937X_EXT_SPK */
};
...
static int wcd937x_soc_codec_probe(struct snd_soc_component *component)
{
	...
	wcd937x->adc_count = 0;
	wcd937x->ear_hphl_pga_count = 0; /* ADD: Audio_Hph_Ear_Aux */
