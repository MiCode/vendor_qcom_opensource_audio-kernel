...
int audio_pkt_release(struct inode *inode, struct file *file)
{
	...
	struct sk_buff *skb;
	unsigned long flags;
	static ktime_t last_close_call_time = 0;
	...
	pr_err("%s: before calling spf_core_apm_close_all\n", __func__);
	ktime_t current_time = ktime_get();
	s64 elapsed_ms = ktime_to_ms(ktime_sub(current_time, last_close_call_time));
	if (last_close_call_time == 0 || elapsed_ms >= 150) {
		pr_err("%s: Adding sleep\n", __func__);
		msleep(150);
	}
	pr_err("%s: calling spf_core_apm_close_all\n", __func__);
	last_close_call_time = current_time;
