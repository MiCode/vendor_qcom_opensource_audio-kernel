...
#define AFE_MI_SP_STEREO_TX_MODULE              (0X1000A501)
#define AFE_MI_SP_STEREO_RX_MODULE              (0X1000A502)
#define MI_SP_STEREO_Tx_V2_MODULE               (0X1000A510)
#define MI_SP_STEREO_Rx_V2_MODULE               (0X1000A520)
				...
				union afe_spkr_prot_config *prot_config)
{
	...
	case AFE_PARAM_ID_AWDSP_RX_SET_ENABLE:
		...
		if(afe_get_topology(AFE_PORT_ID_AWDSP_RX) ==AFE_MI_SP_STEREO_RX_MODULE)
		{
			param_info.module_id = MI_SP_STEREO_Rx_V2_MODULE;
		}
	...
	case AFE_PARAM_ID_AWDSP_TX_SET_ENABLE:
		...
		if(afe_get_topology(AFE_PORT_ID_AWDSP_TX) ==AFE_MI_SP_STEREO_TX_MODULE)
		{
			param_info.module_id = MI_SP_STEREO_Tx_V2_MODULE;
		}
		break;
#endif	/*CONFIG_SND_SOC_AWINIC_AW882XX*/
...
int aw_send_afe_cal_apr(uint32_t param_id, void *buf, int cmd_size, bool write)
{
	...
	struct mem_mapping_hdr mem_hdr;
	struct param_hdr_v3  param_hdr;
	uint32_t topo_id = afe_get_topology(port_id);
	pr_debug("%s: enter\n", __func__);
	pr_info("%s: Get MI SP TX Topology 0x%x in port of 0x%x",
			__func__,topo_id,port_id);
	if (param_id == AFE_PARAM_ID_AWDSP_TX_SET_ENABLE) {
		port_id = AFE_PORT_ID_AWDSP_TX;
		module_id = AFE_MODULE_ID_AWDSP_TX;
		if (topo_id == AFE_MI_SP_STEREO_TX_MODULE)
		{
			module_id = MI_SP_STEREO_Tx_V2_MODULE;
			pr_info("%s: Get MI SP TX module 0x%x from TX Topology 0x%x in port of 0x%x",
			__func__,module_id,topo_id,port_id);
		}
	}
	...
	/* Get MI SP Module ID according to Topology */
	if (topo_id == AFE_MI_SP_STEREO_RX_MODULE)
	{
		module_id = MI_SP_STEREO_Rx_V2_MODULE;
		pr_info("%s: Get MI SP RX module 0x%x from RX Topology 0x%x in port of 0x%x",
			__func__,module_id,topo_id,port_id);
	}
	...
	pr_debug("%s: Sending aw_cal port = 0x%x, topo = 0x%x, module = 0x%x,cal size = %zd, cal addr = 0x%pK\n",
		__func__, port_id,topo_id, module_id,aw_cal->cal_data.size, &aw_cal->cal_data.paddr);
